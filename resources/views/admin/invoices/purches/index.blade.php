                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <a href="{{ route('dashboard.purches_invoices.create') }}" class="btn btn-primary"> اضافة فاتورة </a>
                            </div>
                                            ...
                                            <tbody>
                                                @forelse ($invoices as $invoice)
                                                    <tr>
                                                        ...
                                                        <td> {{ $invoice->telegram }} </td>
                                                        <td> {{ $invoice->whatsapp }} </td>
                                                        <td> <span
                                                                class="badge badge-pill badge-{{ $invoice->status == 'نشط' ? 'success' : 'danger' }}">{{ $invoice->status }}</span>
                                                        </td>
                                                            ...
                                                            <div class="dropdown float-md-left">
                                                                ...
                                                                <div class="dropdown-menu"
                                                                    aria-labelledby="dropdownBreadcrumbButton"><a
                                                                        class="dropdown-item"
                                                                        href="{{ route('dashboard.clients.edit', $invoice->id) }}"><i
                                                                            class="la la-edit"></i> تعديل </a>
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('dashboard.clients.status', $invoice->id) }}"><i
                                                                            class="la la-edit"></i> تعديل الحالة </a>
                                                                    <form
                                                                        action="{{ route('dashboard.clients.destroy', $invoice->id) }}"
                                                                        method="post">
                                                                        @csrf
                                                                        @method('delete')
                                                                        <button type="submit"
